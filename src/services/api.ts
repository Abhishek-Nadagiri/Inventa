...
export const register = async (data: RegisterData): Promise<RegisterResult> => {
  try {
    ...
    if (useSupabase) {
      // Use custom authentication (users table) - no email rate limits!
      ...
      const { error: insertError } = await supabase.from(TABLES.USERS).insert(newUser);
      ...
      // Store current user
      localStorage.setItem('inventa_current_user', JSON.stringify(user));
...
export const login = async (data: LoginData): Promise<LoginResult> => {
  try {
    ...
    if (useSupabase) {
      ...
      const foundUser = userData as DbUser;
      ...
      await recordLoginHistory(foundUser.id, email, foundUser.username, 'success');
...
export const logout = async (): Promise<void> => {
  ...
  localStorage.removeItem('inventa_current_user');
};
...
export const getCurrentUser = async (): Promise<User | null> => {
  try {
    ...
    const cachedUserStr = localStorage.getItem('inventa_current_user');
    if (cachedUserStr) {
      try {
    ...
    if (useSupabase) {
      // Get user from users table using session userId
...
export const uploadDocument = async (userId: string, data: UploadData): Promise<UploadResult> => {
  try {
    ...
    const sessionStr = localStorage.getItem('inventa_session');
    if (!sessionStr) {
      return { success: false, message: 'No active session. Please login again.' };
    }
    ...
    // Get user from localStorage first, then try to fetch if needed
    ...
    // If no cached user, try to get from database
    if (!user) {
    ...
    if (user.id !== userId) {
      return { success: false, message: 'User ID mismatch. Please login again.' };
    }
