...
export const register = async (data: RegisterData): Promise<RegisterResult> => {
  try {
    ...
    if (useSupabase) {
      ...
      // Insert new user
      const { error } = await supabase.from(TABLES.USERS).insert(newUser);
    ...
    const user: User = {
      ...
      privateKey: newUser.private_key_encrypted,
...
export const login = async (data: LoginData): Promise<LoginResult> => {
  try {
    ...
    if (!foundUser || foundUser.password_hash !== passwordHash) {
      await recordLoginHistory(foundUser?.id || null, email, foundUser?.username || '', 'failed', 'Invalid password');
      return { success: false, message: 'Invalid email or password' };
    }
